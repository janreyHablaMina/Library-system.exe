...
import { listBorrowTransactions, listMembers, type BorrowTransaction, type Member } from '../lib/tauriApi'
    ...
    <div className={`min-h-0 flex-1 overflow-auto p-4 ${isDarkMode ? 'bg-[transparent] text-zinc-100' : 'bg-[#f8fafc] text-zinc-900'}`}>
      <section className="p-5">
        ...
        <div className={`mt-4 lg:overflow-visible overflow-hidden rounded-xl border ${isDarkMode ? 'border-zinc-700 bg-[#18181B]' : 'border-zinc-200 bg-white'}`}>
              ...
              <tbody>
                {paginatedTransactions.map((row) => (
                  <tr key={row.id} onClick={() => onOpenTransactionDetail(row.id)} className={`border-t cursor-pointer transition-colors ${isDarkMode ? 'border-zinc-700 hover:bg-[#3F3F46]' : 'border-zinc-100 hover:bg-zinc-50'}`}>
                    ...
                    <td className="px-3 py-3"><p className={`font-semibold ${isDarkMode ? 'text-zinc-100' : 'text-zinc-900'}`}>{row.book}</p><p className={`text-xs ${isDarkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>{row.copyId}</p></td>
                    ...
                    <td className="px-3 py-3 text-right">
                      <button
                        ...
                        className={`inline-grid h-9 w-9 place-items-center rounded-lg transition-colors ${
                          isDarkMode
          ...
          <div className={`relative z-0 flex flex-wrap items-center justify-between gap-3 rounded-b-xl border-t px-4 py-3 text-sm ${isDarkMode ? 'border-zinc-700 bg-[#18181B] text-zinc-300' : 'border-zinc-200 bg-white text-zinc-600'}`}>
