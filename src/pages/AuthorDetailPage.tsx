...
export function AuthorDetailPage({ isDarkMode, onBack, authorId }: Props) {
  ...
  return (
    <div className={`flex-1 overflow-y-auto min-h-0 w-full ${isDarkMode ? 'bg-[transparent] text-zinc-100' : 'bg-[#f8fafc] text-zinc-900'}`}>
      ...
      {bookToDelete ? (
        <div className="fixed inset-0 z-[140] flex items-center justify-center p-4 bg-zinc-950/60 backdrop-blur-sm">
      ...
      <div className="max-w-[1600px] mx-auto p-6 space-y-5">
        <div className="flex items-center justify-between gap-4">
          <button onClick={onBack} className={`inline-flex items-center gap-2 text-sm font-semibold ${isDarkMode ? 'text-zinc-300 hover:text-white' : 'text-zinc-600 hover:text-zinc-900'}`}>
            <ArrowLeft size={16} />
            Back to Authors
          </button>
          ...
          <div className="p-6 xl:p-8 grid grid-cols-1 xl:grid-cols-[1.5fr_1fr] gap-8">
            <div className="flex flex-col sm:flex-row sm:items-center gap-8 min-w-0">
              ...
              <div className="min-w-0 flex-1">
                <h2 className={`text-[35px] font-semibold tracking-tight ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}>{detail.name}</h2>
                {loading ? <p className={`mt-1 text-xs ${isDarkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>Loading author details...</p> : null}
                <div className="mt-1 flex items-center gap-2">
                  <p className={isDarkMode ? 'text-zinc-400' : 'text-zinc-500'}>Author Profile</p>
                  <button onClick={copyAuthorName} className={isDarkMode ? 'text-zinc-400 hover:text-emerald-400' : 'text-zinc-500 hover:text-emerald-600'}>
                    <Copy size={13} />
                  </button>
                ...
                <div className={`mt-5 space-y-2 text-sm ${isDarkMode ? 'text-zinc-300' : 'text-zinc-700'}`}>
                  <p className="flex items-center gap-2.5"><Mail size={15} className={isDarkMode ? 'text-zinc-400' : 'text-zinc-500'} />{detail.email}</p>
                  <p className="flex items-center gap-2.5"><Globe size={15} className={isDarkMode ? 'text-zinc-400' : 'text-zinc-500'} />{detail.nationality}</p>
                </div>
              </div>
            </div>
        ...
        <section className="grid grid-cols-1 xl:grid-cols-[320px_1fr] gap-4">
          <div className={cardClass}>
            ...
            <div className="p-5">
              <div className={`rounded-xl p-4 min-h-[190px] ${isDarkMode ? 'bg-zinc-900/40' : 'bg-zinc-50'}`}>
                <Quote size={20} className="text-emerald-500" />
                <p className={`mt-4 text-sm leading-relaxed ${isDarkMode ? 'text-zinc-300' : 'text-zinc-700'}`}>{detail.biography}</p>
              </div>
            </div>
          </div>
                      ...
                      <tr key={book.id} className={`border-t ${isDarkMode ? 'border-zinc-700 hover:bg-[#3F3F46]' : 'border-zinc-100 hover:bg-zinc-50'}`}>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className={`font-semibold text-sm ${isDarkMode ? 'text-zinc-100' : 'text-zinc-900'}`}>{book.title}</p>
                              <p className={`text-[11px] ${isDarkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>ISBN: {book.isbn || '-'}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          <span className={`rounded-md px-2 py-1 text-xs font-semibold ${
                            book.isArchived 
                              ? (isDarkMode ? 'bg-zinc-500/30 text-zinc-100 border border-zinc-500/30' : 'bg-zinc-200 text-zinc-800 border border-zinc-300')
                              : book.available > 0
                        ...
                        <td className={`px-4 py-3 font-semibold ${isDarkMode ? 'text-zinc-100' : 'text-zinc-800'}`}>{book.available} / {book.totalCopies}</td>
        ...
        <section className={`rounded-2xl border px-6 py-5 flex items-center justify-between ${isDarkMode ? 'border-emerald-900/40 bg-emerald-950/20' : 'border-emerald-100 bg-emerald-50/60'}`}>
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className={`text-base font-bold ${isDarkMode ? 'text-emerald-200' : 'text-zinc-800'}`}>Author Management</p>
              <p className={`text-sm ${isDarkMode ? 'text-emerald-100/80' : 'text-zinc-600'}`}>
